...
In my code, when running `python scripts/demo.py`, the arm first tries to place the red tube on Rack B but misses and shows `FAILED`. It then retries, picks the tube back up, and places it correctly on Rack B. Finally, it moves the blue tube to Rack A in one successful attempt.
